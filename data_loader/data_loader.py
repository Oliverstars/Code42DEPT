...
import os
import torch
from .customer_tokenize import get_tokenizer
import numpy as np
...
class Dataset_2DEPT(Dataset):
    def __init__(self, config, prefix, is_test, tokenizer):
        self.config = config
        self.prefix = prefix
        self.is_test = is_test
        self.tokenizer = tokenizer
        self.json_data = json.load(open(config.data_path + '/' + prefix + '.json'))
        self.rel2id = json.load(open(config.data_path + '/rel2id.json'))[1]
            ...
            if s2ro_map:
                ...
                for s in s2ro_map:
                    ...
                    for ro in s2ro_map.get((sub_head, sub_tail), []):
                        ...
                        triple_matrix[relation, sub_head, obj_head:obj_tail + 1] = np.ones(obj_tail + 1 - obj_head)
                        triple_matrix[relation, sub_head:sub_tail + 1, obj_head] = np.ones(sub_tail + 1 - sub_head)
                        triple_matrix[relation, sub_head, obj_head] = 2
...
def collate_fn_2DEPT(batch):
    ...
    # if use WebNLG_star, modify 24 to 171; 216 for WebNLG
...
def get_loader(config, prefix, is_test=False, num_workers=0, collate_fn=collate_fn_2DEPT):
    tokenizer = get_tokenizer(config.root_path + '/pre_trained_bert/vocab.txt')
    dataset = Dataset_2DEPT(config, prefix, is_test, tokenizer)
    if not is_test:
